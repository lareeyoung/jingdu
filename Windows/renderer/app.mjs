...
document.addEventListener('change',event=>{try{const id=event.target.id;if(state.busy&&id.startsWith('note-')){renderNotes();return;}if(id==='speed'){state.rate=Number(event.target.value);video.playbackRate=state.rate;}if(id==='layout'){state.layout=event.target.value;$('.workspace').className='workspace '+(state.layout==='balanced'?'balanced':state.layout==='vertical'?'vertical':'');}if(id==='follow'){state.follow=event.target.checked;currentCueId=null;updateTime();}if(id==='script-range'){let start=0,end=Math.min(duration(),300);if(event.target.value==='shot'){start=state.dialogShot.start;end=state.dialogShot.end}else if(event.target.value==='custom'){start=state.rangeIn??state.time;end=state.rangeOut??Math.min(duration(),start+30)}$('#script-start').value=start.toFixed(3);$('#script-end').value=end.toFixed(3)}if(['note-start','note-end','note-track'].includes(id)){const note=p().notes.find(x=>x.id===state.noteId);if(editingNote!==note.id){checkpoint();editingNote=note.id;}const start=Number($('#note-start').value),end=Number($('#note-end').value);if(start<0||end<=start||end>duration()){renderNotes();throw new Error('笔记区间应在视频内，且终点晚于起点。')}note.start=start;note.end=end;note.track=$('#note-track').value;touch();renderTimeline();}}catch(e){toast(e.message,true);}});
